...
- **Never detects a type change, unless explicitly hinted — and even then, only on Postgres.** Same underlying problem as above: a live column's `type_name` is dialect-native, verbatim text with no portable representation to compare `ColumnType` against without reimplementing `automap::rust_type_for`'s heuristic in reverse, per dialect. `AutogenerateOptions::changed_column_types` lets you confirm "this table's column genuinely changed type" up front — a `(table, column)` pair, acted on only when that column is actually live *and* still expected under the same name (a rename or add/drop is a different hint) — producing a single `AlterTable::alter_column_type`. Unlike renames, though, this only ever emits a statement on Postgres: MySQL/MariaDB's `MODIFY COLUMN` would need the column's *entire* definition restated (nullability, default, ...) or those get silently reset, and SQLite has no direct column-type-alter support at all (it'd need a full create-copy-drop-rename table rebuild) — `diff` checks `Dialect::supports_alter_column_type()` first and quietly skips the hint on either of those rather than emitting something wrong.
...
let options = AutogenerateOptions {
    ...
    changed_column_types: vec![("users".to_string(), "signup_count".to_string())],
};
let statements = engine
...
`AlterTable` also gained a third operation for this, `rename_column`, spelled identically (`ALTER TABLE t RENAME COLUMN old TO new`) on all three dialects — SQLite has supported it since 3.25.0, MySQL/MariaDB since 8.0/10.5.2, both well below anything else this crate assumes — plus a fourth, Postgres-only `alter_column_type`, rendering `ALTER TABLE t ALTER COLUMN c TYPE ty USING c::ty` (the `USING` cast lets it also handle conversions Postgres won't perform implicitly, like text to integer). Calling `alter_column_type(...).to_sql(...)` against a dialect where `Dialect::supports_alter_column_type()` is `false` panics — the same "wrong operation for this dialect" category of misuse `.not_null()` after `.drop_column()` already panics for — which `diff`'s own capability check is specifically there to avoid ever reaching.
...
`(price > 50 || quantity > 10) && discount < 20` is a grouping that's unreachable without explicit parens — `&&` always binds tighter than `||` in the flat form, so the only reachable unparenthesized reading would be `price > 50 || (quantity > 10 && discount < 20)`, a genuinely different condition. A parenthesized group is only ever valid at the top level of the expression or as an operand of `&&`/`||`, though — nesting one inside arithmetic, another comparison, or a string function's argument (`(price > 50) + 1`, `(price > 50) > 2`, `upper((price > 50))`) is rejected at macro-expansion time rather than left to fail (confusingly) at Rust compile time, since the query builder's `Expr` has no type distinguishing "arithmetic" from "boolean" the way Rust itself does — a case like that would still build a syntactically valid SQL `Expr` tree even though the Rust side hard-fails to compile.

`upper(x)`, `lower(x)`, and `concat(a, b)` — plus double-quoted string literals — are also supported, mapping onto Rust's `.to_uppercase()`/`.to_lowercase()`/`format!("{}{}", ...)` and the query builder's own `Expr::upper()`/`Expr::lower()`/`Expr::concat()`:
...
Both infer `ty = "String"` automatically, same as a comparison infers `bool`. Their arguments are ordinary arithmetic sub-expressions (so `upper(...)`/`lower(...)`/`concat(...)` can nest inside each other, or appear as a comparison operand), but a call itself can't be chained with `+`/`-`/`*`/`/` the way two fields can — write `concat(a, b)`, not `a + b` for strings.

Still deliberately nothing richer beyond that (no `CASE`/`COALESCE`, or referencing another table's columns). `CASE`/`COALESCE` specifically are skipped rather than deferred-by-oversight: they fundamentally operate on NULL-able SQL values (`Option<T>` on the Rust side), but this design's arithmetic operators implicitly assume plain, non-`Option` field types — there's no natural "first non-null" analog once a value might not be an `Option`, so supporting them properly is a nullability design question, not just a parser extension. String functions have no such conflict, which is why they're supported and `CASE`/`COALESCE` aren't (yet). `#[hybrid(...)]` is repeatable (stack as many as a struct needs) and struct-level, not field-level, since a hybrid property isn't backed by a column of its own; `ty` is optional, inferred from the first field the expression references when omitted (or as `bool` for a comparison or `&&`/`||` chain, or `String` for a string function), or given explicitly (`ty = "i64"`) when that inference would guess wrong. `total_expr()`/`is_expensive_expr()`/etc. are usable anywhere an `Expr` is accepted — `.filter()`, `.columns()` — but not `.order_by()`, which only accepts a bare `Column` today, not an arbitrary expression.
...
`rusty_db::relations::load_many_joined`/`load_has_one_joined`/`load_one_joined`/`load_many_to_many_joined` are a third strategy (SQLAlchemy calls this `joinedload`), covering all four relationship shapes: unlike the two above, it doesn't add a second query at all — both sides come back from a single `LEFT JOIN` (a two-hop one, through the join table, for `load_many_to_many_joined`), one round trip total:

```rust
let filter = Some(User::table().col("active").eq(true));
let (users, orders_by_user): (Vec<User>, HashMap<i64, Vec<Order>>) =
    rusty_db::relations::load_many_joined(&engine, filter, "id", "user_id").await?;

let (users, profile_by_user): (Vec<User>, HashMap<i64, Profile>) =
    rusty_db::relations::load_has_one_joined(&engine, None, "id", "user_id").await?;

let (orders, user_by_id): (Vec<Order>, HashMap<i64, User>) =
...
let (posts, tags_by_post): (Vec<Post>, HashMap<i64, Vec<Tag>>) =
    rusty_db::relations::load_many_to_many_joined(
        &engine, None, "id", "post_tags", "post_id", "tag_id", "id",
    )
    .await?;
```

That's a materially different shape than `load_many`/`load_many_via_subquery` return — those always start from an already-fetched (or independently queried) batch on one side, so they only ever need to return the *other* side. A joined query, by contrast, repeats whichever row a match can point back to more than once — so all four of these fetch *and* return both sides themselves, deduplicating whichever one the join naturally repeats. For `load_many_joined`/`load_has_one_joined`/`load_many_to_many_joined` that's the parent (a parent row repeats once per matching child/target, and once, with every counterpart column `NULL`, for a parent with none) — alongside the same `HashMap<PK, Vec<Child>>`/`HashMap<PK, Child>`/`HashMap<PK, Vec<Target>>` shapes `load_many`/`load_has_one`/`load_many_to_many` return (a childless/targetless parent still appears in the parent list, but has no entry in the map; `load_has_one_joined` reports the identical `Error::Conflict` for more than one matching row per parent). `load_one_joined` flips that: every `Order` row is distinct (a child only ever matches at most one parent, so there's nothing to deduplicate there), but more than one child can reference the *same* parent — so it's the returned `HashMap<PK, Parent>` that deduplicates instead, the same shape `load_one` already returns.

They also solve a problem neither of the other two strategies ever runs into: the two mapped types' own column names can collide (both mapping their own `id` primary key, say). Every selected column gets an internal, uniquely prefixed alias that `FromRow` decodes back through — invisible to the caller, and safe regardless of whether the two tables happen to share column names; `load_many_to_many_joined`'s join table itself is never selected from beyond its two join columns, so it can't be part of that collision either.

Each of these only accepts a plain, optional `filter` on one side's own table (`Parent`'s for the first two and for `load_many_to_many_joined`, `Child`'s for `load_one_joined` — build it with `.table().col(...)`, not a fresh `Table::new(...)`, so it resolves against the same table name this function builds internally) — a `_joined_from_query` sibling next to each one instead takes an arbitrary caller-built `Select` (its own filters, joins, even other CTEs), the same widened contract `_via_subquery` already has:

```rust
let users_table = User::table();
let orders_table = Order::table();
let big_spenders = Select::from(&users_table)
    .join(&orders_table, users_table.col("id").eq_col(&orders_table.col("user_id")))
    .columns(User::COLUMNS.iter().map(|c| users_table.col(*c)))
    .filter(orders_table.col("amount").gt(1000_i64))
    .distinct();

let (users, orders_by_user): (Vec<User>, HashMap<i64, Vec<Order>>) =
    rusty_db::relations::load_many_joined_from_query(&engine, big_spenders, "id", "user_id").await?;
```

The one added requirement `_joined_from_query`'s `Select` carries that `_via_subquery`'s doesn't: it must select every column of the "from" side's own `Mapped::COLUMNS`, each under its own (unaliased) column name — not just a single key column — since reconstructing a full mapped row (not just a key) is what the `LEFT JOIN` needs to build. It's wrapped as a `WITH` CTE (the same trick `_via_subquery` uses) and joined against directly instead of a plain `Table::new(Parent::TABLE_NAME)` — that CTE indirection is exactly what lets the caller bring their own joins/CTEs, something a plain `filter` can't do.

`#[has_many(...)]`/`#[has_one(...)]`/`#[belongs_to(...)]`/`#[many_to_many(...)]` each also generate `_joined`- and `_joined_from_query`-suffixed convenience methods around the matching functions above, alongside their existing select-in and `_via_subquery` methods — same as `_via_subquery`, no separate `strategy = "..."` parameter, all methods are always generated together:

```rust
let filter = Some(User::table().col("active").eq(true));
let (users, orders_by_user): (Vec<User>, HashMap<i64, Vec<Order>>) =
    User::load_orders_joined(&engine, filter).await?;

let (users, orders_by_user): (Vec<User>, HashMap<i64, Vec<Order>>) =
    User::load_orders_joined_from_query(&engine, big_spenders).await?;
```

The parent primary key column (`has_many`/`has_one`/`many_to_many`) or the `foreign_key` column named in the attribute (`belongs_to`) is filled in automatically, the same way the select-in and `_via_subquery` methods' key columns already are — `filter` is the only thing the caller supplies, built against `Self::table()`/`Child::table()` as described above.
...
`rusty_db::migration::cli` is a thin `up`/`down`/`status` subcommand dispatcher over `Migrator`, for an Alembic-style command-line workflow (`cargo run --bin migrate -- up`) without inventing a file-based migration format: migrations here are always compile-time Rust `const` arrays, so there's nothing for a generic, project-agnostic binary to discover on disk. Instead, it's meant to back a few lines of a small binary *in your own crate*, which already has both the `&[Migration]` array and however it builds its own `Engine`:
...
async fn main() -> rusty_db::Result<()> {
    ...
    rusty_db::migration::cli::run(&engine, MIGRATIONS).await
}
```

`run` reads `std::env::args()` and prints to `stdout` — zero extra dependencies, not even an argument-parsing crate. `up` prints each newly-applied version (or "Already up to date."); `down` reverts the single most-recently-applied migration and prints which version, or "Nothing to revert."; `status` lists every migration with an `[x]`/`[ ]` mark. Anything else — including no subcommand at all — is `Err(Error::Migration(_))`, the same error every other migration failure in this crate already uses. `run_to` is the same logic with the arguments and output writer passed in explicitly instead of read from the real process — what `run` itself calls, and the seam that makes this testable without capturing actual stdout or depending on real argv.

## Status

This covers Core (query builder — including aggregate functions/expression columns via `SelectExpr`, `GROUP BY`/`HAVING`, set operations (`UNION`/`UNION ALL`/`INTERSECT`/`EXCEPT`) via `SetOperation`, SQL functions/arithmetic/`CASE`/`COALESCE`, subqueries (`IN (subquery)`, correlated `EXISTS`, scalar subqueries), CTEs (`WITH`/`WITH RECURSIVE` via `Cte`), and window functions (`ROW_NUMBER`/`RANK`/`DENSE_RANK`, and aggregates as window functions, via `.over(...)`/`Window`), a portable DDL builder (`CreateTable`/`DropTable`/`CreateIndex`/`DropIndex`/`AlterTable` — including `rename_column`, spelled identically on all three dialects — with a portable `ColumnType`), and Alembic-style autogenerate diffing a mapped type's expected shape against a live database (`Engine::autogenerate_migration`, `AutogenerateOptions` opting into caller-hinted column renames, allow-listed whole-table drops, and caller-confirmed column type changes (Postgres only, via a new `AlterTable::alter_column_type`) beyond the conservative add/drop-column default) — connections (pooling/tuning, connection-level event hooks on connect/checkout/checkin, tunable per-connection statement-cache capacity, streaming query results via `fetch_stream`/`fetch_stream_as`, TLS, read replicas, sharding via `ShardRouter` (naive modulo or consistent-hash routing), query timeouts), first-class `Uuid`/`BigDecimal`/`Json`/temporal (`NaiveDate`/`NaiveTime`/`NaiveDateTime`/`DateTime<Utc>`)/array (`Vec<T>`) value types), a thin mapping layer (`#[derive(Mapped)]`, `#[derive(MappedEnum)]`/`#[derive(MappedNewtype)]` for custom field types, joins, has-many/has-one/belongs-to/many-to-many select-in eager loading plus a generated `_via_subquery` convenience method for each (joining directly against a caller-supplied `Select` wrapped as a CTE instead of shipping a parent key list back and forth — also callable directly as `rusty_db::relations::load_many_via_subquery`/`load_has_one_via_subquery`/`load_one_via_subquery`/`load_many_to_many_via_subquery`) and a generated `_joined`/`_joined_from_query` convenience method pair for each, a third strategy covering all four relationship shapes (`rusty_db::relations::load_many_joined`/`load_has_one_joined`/`load_one_joined`/`load_many_to_many_joined`, a single `LEFT JOIN` round trip (a two-hop one for `many_to_many`) returning both sides, deduplicating whichever one the join naturally repeats — plus a `_from_query`-suffixed sibling of each accepting an arbitrary caller-built `Select`, the same widened contract `_via_subquery` has, wrapped as a CTE the same way), cascade delete/orphan rules, mapping-level column defaults via `#[table(default = "...")]`, computed/hybrid properties via `#[hybrid(name = "...", expr = "...")]` (arithmetic, comparisons, `&&`/`||` chains of comparisons with parenthesized grouping, and `upper`/`lower`/`concat` string functions — no `CASE`/`COALESCE` yet, a deliberate scope cut over a design gap around `Option`/nullability rather than an oversight), versioned migrations (standalone via `Migrator`, or folded into a session's transaction via `session.migrate`, plus a dependency-free `up`/`down`/`status` CLI dispatcher, `rusty_db::migration::cli`, for a small `src/bin/migrate.rs` in your own crate), automap-style struct generation from schema reflection (`Engine::automap_table`/`automap_all`), a hand-implemented `Lifecycle` trait for entity-level before/after/`validate` hooks (`Session::add_mut`/`update_mut`/`delete_mut`), and a unit-of-work `Session` with autoflush and an identity map (including eviction on delete). Three drivers exist — SQLite, PostgreSQL, and MySQL/MariaDB — all built the same way (wrapping `sqlx`) and all exercised by the test suite. The Postgres and MySQL tests run against real servers when reachable (`POSTGRES_TEST_URL`/`MYSQL_TEST_URL`, defaulting to local `rusty`/`rusty` test databases) and just skip themselves rather than fail if one isn't — so `cargo test` stays green without either installed, but this environment does have both, and both are actually exercised here.
...
It also covers `load_many_joined`: against fixture data where `User` and `Order` both map their own `id` column (proving the internal per-side column aliasing actually resolves the collision, not just that this particular schema happens not to trigger it), every parent comes back exactly once — including a childless one, correctly deduplicated from the join's natural per-child repetition — with its children grouped into the same map shape `load_many` returns, and a childless parent has no entry in it; a separate test confirms a `filter` narrows which parents (and therefore which children) come back at all, the same "excluded by the filter, not by having no children" distinction the subqueryload test makes. `load_has_one_joined` gets the same collision-resolving coverage (against `User`/`Profile`, which also both map their own `id`) plus the identical `Error::Conflict` check `load_has_one` has, for the same not-really-1:1 data. `load_one_joined` gets the flipped-shape version: against `Order`/`User` (also both mapping `id`), every order comes back exactly once with no dedup needed (unlike the other two, where deduplication happens on the *parent* side), while two orders sharing the same `user_id` still collapse to one entry in the returned user map — plus a separate test confirming a `filter` on `Order`'s own table narrows which children (and therefore which referenced users) come back. `load_many_to_many_joined` rounds it out: against `Post`/`Tag` (also both mapping `id`), every post comes back exactly once — including an untagged one — deduplicated from the join's natural per-tag repetition, with tags grouped into the same map shape `load_many_to_many` returns and no entry for the untagged post; a separate test confirms a `filter` on `Post`'s own table narrows which posts (and therefore which tags) come back. A further test exercises all four *derive-generated* `_joined` methods directly (`User::load_orders_joined`, `User::load_profile_joined`, `Order::load_user_joined`, `Post::load_tags_joined`) rather than the plain functions they wrap, confirming the parent primary key or `foreign_key` column the macro fills in automatically is correct end to end — the same check the `_via_subquery` derive test above makes for that strategy. `load_many_joined_from_query` gets a dedicated test proving the widened contract genuinely accepts the caller's own `JOIN` (not just a relabeled `filter`): a `Select` joining `users` against `orders` and filtering on the order amount narrows the parent batch to exactly the users with a big enough order, something a plain `filter` on `User`'s own table alone couldn't express. A further test exercises all four *derive-generated* `_joined_from_query` methods the same way the plain `_joined` derive test does, each given a full-column-projection `Select` (one of them additionally narrowed by the caller's own `.filter(...)`), confirming the widened contract is wired all the way through.
...
`tests/autogenerate.rs` (SQLite) covers `Engine::autogenerate_migration`/`TableSpec` end to end: a table missing entirely gets exactly one `CreateTable` statement (including its primary key), which — once executed — is genuinely usable through ordinary `#[derive(Mapped)]` inserts/selects, and re-diffing afterward finds nothing left to do; an already-up-to-date table generates nothing; a field added to the struct becomes a single `AlterTable::add_column`, and a field removed from it becomes a single `AlterTable::drop_column` — both verified by actually applying the statement and re-diffing (through a fresh `Engine`, per `AlterTable`'s own docs) to confirm convergence; a live table with no corresponding entry in `expected` at all (standing in for this crate's own migration-bookkeeping table, or a table tracked elsewhere) is never mentioned in the generated statements and is left completely untouched, whether or not any `expected` tables are actually present; a matching `AutogenerateOptions::renamed_columns` hint produces a single `AlterTable::rename_column` instead of a data-losing drop-plus-add, verified by actually applying it and confirming the row's pre-rename data survived under the new column name; and a table named in `AutogenerateOptions::allow_drop_tables` gets a real `DropTable` when it's live but absent from `expected` — while the exact same live table, with the option left off, is correctly left alone. `tests/autogenerate_postgres.rs`/`autogenerate_mysql.rs` are reduced single-test versions confirming the same generate-apply-reconverge round trip produces genuinely dialect-correct DDL there too (a nullable `Uuid`/`Json` field on each, respectively); `autogenerate_postgres.rs` additionally covers a matching `AutogenerateOptions::changed_column_types` hint against a live Postgres server — a column deliberately created narrower (`INTEGER`) than what the mapped type expects (`BIGINT`) generates no statement at all without the hint, generates exactly one `AlterTable::alter_column_type` with it, and — applied against a table already holding a row — the `USING` cast preserves that row's data and the column's live type (checked via a fresh `Engine::table_schema` reflection) is genuinely `bigint` afterward; a further check confirms the hint doesn't self-invalidate the way a rename hint does, since there's no cheap way to tell "already applied" from "still needs applying" without the reverse-heuristic this feature exists specifically to avoid. The type-inference side (`#[derive(Mapped)]` mapping each documented field type — including `Option<T>` nullability, `Vec<u8>` vs. any other `Vec<T>`, and an unrecognized custom type — to its portable `ColumnType`) is unit-tested directly in `crates/rusty-db-derive/src/lib.rs`; the pure diff logic (create/add-column/drop-column decisions, a matching rename hint pre-empting the ordinary add/drop pair, a stale/irrelevant rename hint being ignored and falling back to add/drop, an allow-listed vs. non-allow-listed table drop decision, a matching type-change hint producing an `alter_column_type` on Postgres, an unhinted or stale hint producing nothing, and — critically — the exact same matching hint producing nothing at all on a dialect where `Dialect::supports_alter_column_type()` is `false`) is unit-tested directly in `crates/rusty-db-core/src/autogenerate.rs`; `AlterTable::rename_column`'s identical-across-dialects rendering (and that `.not_null()`/`.default_raw(...)` still panic after it, same as after `.drop_column(...)`), plus `alter_column_type`'s Postgres-only `USING`-cast rendering and its panic on SQLite/MySQL, are unit-tested in `crates/rusty-db-core/src/query/tests.rs`.
...
`tests/hybrid_properties.rs` (SQLite) covers `#[hybrid(...)]`: the generated plain-Rust method computes the correct value from an instance's own fields; the generated `_expr()` method filters real rows to exactly the same set the Rust-side computation would select, for both a simple `price * quantity` expression and a parenthesized, explicit-`ty` one (`(price * quantity) - discount`); it works as a `.columns()` entry too, with its value matching each row's own Rust-side computation exactly. A `price > 50` comparison hybrid is covered separately: its Rust-side method returns the correct `bool` for both a value under and over the threshold, and its `_expr()` — used directly as a `.filter()` condition, with no further comparison chained onto it — selects real rows matching exactly what the Rust-side `bool` would keep, against sample data deliberately containing both a match and a non-match (so a filter that's silently always-true or always-false would be caught, not just a filter that happens to match everything). A `price > 5 && quantity > 1` hybrid gets the same treatment, confirming the `&&`-combined `_expr()` selects exactly the rows the Rust-side `&&` would keep. `upper(sku)`/`lower(sku)` hybrids are checked both as `.columns()` entries (matching `.to_uppercase()`/`.to_lowercase()` row-for-row) and as a `.filter()` condition matching mixed-case data case-insensitively; a `concat(sku, "-item")` hybrid is checked the same way against `format!("{}{}", ...)`. A `(price > 50 || quantity > 10) && discount < 20` hybrid is checked against fixture data chosen specifically so the grouped and the flat (unparenthesized) reading of the same operators disagree — proving the parenthesized grouping is what's actually driving the result, not just that the expression happens to evaluate to a correct-looking boolean. The parser/codegen themselves (operator precedence and left-associativity, parenthesized grouping, unsuffixed-literal codegen so a literal adapts to the referenced field's own numeric type, unknown-field and malformed-expression rejection, that the SQL side qualifies by column name rather than Rust field name, every comparison operator tokenizing/parsing/rendering its matching `Expr` method, a comparison's operands accepting arbitrary arithmetic rather than just bare fields, chained comparisons being rejected, a bare `=`/`&`/`|` never being silently treated as `==`/`&&`/`||`, `&&` binding tighter than `||`, both being left-associative, a comparison or `&&`/`||` chain inferring `bool` rather than an operand's own type when `ty` is omitted, string literal tokenizing/parsing and unterminated-string rejection, `upper`/`lower`/`concat` parsing as function calls with the right arity, wrong-arity and unknown-function-name rejection, a string function defaulting to `ty = "String"`, a parenthesized comparison/boolean group overriding the default flat precedence, and a comparison or `&&`/`||` combinator smuggled into an arithmetic operand, another comparison, or a string function's argument via parentheses being rejected at macro-expansion time) are unit-tested directly in `crates/rusty-db-derive/src/lib.rs`.

`tests/migration_cli.rs` (SQLite) covers `rusty_db::migration::cli::run_to` (used directly rather than `run`, since it's the version that takes arguments and an output writer explicitly instead of reading real process argv/stdout — the seam that makes this testable at all): `up` prints each newly-applied migration's version and, on a second call with nothing left pending, "Already up to date."; `down` prints the version it reverted, then "Nothing to revert." once every migration has been undone; `status` marks exactly the migrations that are actually applied (checked by counting `[x]` marks, not just checking both migrations' names appear in the output); and both an unrecognized subcommand and no subcommand at all are `Err(Error::Migration(_))`, the same error type every other migration failure in this crate already uses.
