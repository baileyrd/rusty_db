...
`rusty_db::relations::load_many_joined`/`load_has_one_joined` are a third strategy (SQLAlchemy calls this `joinedload`), for the `has_many`/`has_one` shapes so far: unlike the two above, it doesn't add a second query at all — parents and their matching children come back from a single `LEFT JOIN`, one round trip total:
...
That's a materially different shape than `load_many`/`load_many_via_subquery` return — those always start from an already-fetched (or independently queried) parent batch, so they only ever need to return the *children*. A joined query, by contrast, repeats a parent row once per matching child (and once, with every child column `NULL`, for a parent with none) — so both of these fetch *and* return the deduplicated parents themselves, alongside the same `HashMap<PK, Vec<Child>>`/`HashMap<PK, Child>` shapes `load_many`/`load_has_one` return (a childless parent still appears in the parent list, but has no entry in the map; `load_has_one_joined` reports the identical `Error::Conflict` for more than one matching row per parent).

They also solve a problem neither of the other two strategies ever runs into: `Parent`'s and `Child`'s own column names can collide (both mapping their own `id` primary key, say). Every selected column gets an internal, uniquely prefixed alias that `FromRow` decodes back through — invisible to the caller, and safe regardless of whether the two tables happen to share column names.

Simpler than `load_many_via_subquery`'s arbitrary caller-built `Select` — these only accept a plain, optional `filter` on `Parent`'s own table (build it with `Parent::table().col(...)`, not a fresh `Table::new(...)`, so it resolves against the same table name this function builds internally), since the join and column-aliasing they need require an actual `Table` handle this function controls throughout. Not yet generalized to `belongs_to`/`many_to_many`, or wired into `#[has_many(...)]`/`#[has_one(...)]` as an opt-in strategy the way select-in/subqueryload already are — call these directly.
