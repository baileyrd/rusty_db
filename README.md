...
What the DDL builder overall still doesn't cover: renaming a column or table, or altering an existing column's type/constraints — `ADD`/`DROP` only. This — along with `AlterTable` itself now existing — is a prerequisite for (not the same thing as) Alembic-style autogenerate, which still doesn't exist.
