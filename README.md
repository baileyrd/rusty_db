...
`COUNT`/`SUM`/`AVG`/`MIN`/`MAX` are ANSI-standard and render identically on every backend — no dialect-specific handling needed, unlike `ILIKE` above. There's no `GROUP BY`/`HAVING` yet, so an aggregate mixed with a non-aggregated column only makes sense today when a `.filter(...)` has already narrowed the result to what would otherwise be one group (as in the `customer`/`total` example above, once filtered down to a single customer) — grouping arbitrarily is tracked separately.
