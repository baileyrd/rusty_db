...
Also bool-typed and its `_expr()` also a ready-to-filter condition, built via `Expr::and`/`Expr::or`. Unlike arithmetic parentheses, a `&&`/`||` chain can't itself be wrapped in its own parenthesized group (`(price > 50) && (quantity > 1)` isn't supported) — just a flat, left-to-right chain of comparisons at the very top of the expression.
