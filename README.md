...
`rusty_db::relations::load_many_joined`/`load_has_one_joined`/`load_one_joined` are a third strategy (SQLAlchemy calls this `joinedload`), for the `has_many`/`has_one`/`belongs_to` shapes so far: unlike the two above, it doesn't add a second query at all — both sides come back from a single `LEFT JOIN`, one round trip total:
