...
`rusty_db::relations::load_many_joined` is a third strategy (SQLAlchemy calls this `joinedload`), for the `has_many` shape so far: unlike the two above, it doesn't add a second query at all — parents and their matching children come back from a single `LEFT JOIN`, one round trip total:
