...
//! Same coverage as `replica_set.rs` (SQLite), against a real
//! MySQL/MariaDB server. There's no way to spin up real MySQL replication
...
impl Driver for DownDriver {
    ...
    fn dialect(&self) -> &dyn rusty_db::Dialect {
        static DIALECT: rusty_db::dialect::MySqlDialect = rusty_db::dialect::MySqlDialect;
        &DIALECT
    }
}
...
async fn a_down_replica_fails_over_to_the_next_healthy_one() -> rusty_db::Result<()> {
    ...
    node_table(&primary, "replica_set_mysql_primary", "primary").await?;
    node_table(
        &healthy_replica,
        "replica_set_mysql_healthy",
        "replica-healthy",
    )
    .await?;
    ...
    let table = Table::new("replica_set_mysql_healthy");
    for _ in 0..4 {
        let row = set
            .fetch_one(&Select::from(&table).filter(table.col("id").eq(1_i64)))
            .await?;
    ...
    set.primary()
        .connect()
        .await?
        .execute("DROP TABLE replica_set_mysql_primary", &[])
        .await?;
    set.primary()
        .connect()
        .await?
        .execute("DROP TABLE replica_set_mysql_healthy", &[])
        .await?;
...
async fn all_replicas_down_falls_back_to_the_primary() -> rusty_db::Result<()> {
    ...
    node_table(&primary, "replica_set_mysql_all_down", "primary").await?;

    let set = ReplicaSet::with_replicas(primary, vec![down_engine(), down_engine()]);

    assert_eq!(
        label_in(set.primary(), "replica_set_mysql_all_down").await?,
        "primary"
    );
    let table = Table::new("replica_set_mysql_all_down");
    let row = set
        .fetch_one(&Select::from(&table).filter(table.col("id").eq(1_i64)))
        .await?;
    ...
    set.primary()
        .connect()
        .await?
        .execute("DROP TABLE replica_set_mysql_all_down", &[])
        .await?;
...
async fn writes_always_go_to_the_primary_never_a_replica() -> rusty_db::Result<()> {
    ...
    node_table(&primary, "replica_set_mysql_writes_primary", "primary").await?;
    node_table(&replica, "replica_set_mysql_writes_replica", "replica").await?;
    let replica_handle = replica.clone();

    let set = ReplicaSet::with_replicas(primary, vec![replica]);

    let table = Table::new("replica_set_mysql_writes_primary");
    set.execute(
    ...
    let replica_table = Table::new("replica_set_mysql_writes_replica");
    ...
    set.primary()
        .connect()
        .await?
        .execute("DROP TABLE replica_set_mysql_writes_primary", &[])
        .await?;
    replica_handle
        .connect()
        .await?
        .execute("DROP TABLE replica_set_mysql_writes_replica", &[])
        .await?;
