...
//! Same coverage as `query_timeout.rs` (SQLite), against a real Postgres
//! server. Postgres has a built-in `pg_sleep()`, so these use that
//! directly for a genuinely slow (not simulated) query to cancel, rather
//! than the lock-contention trick the SQLite version needs (SQLite has
//! no server-side sleep function).
...
async fn an_operation_finishing_within_the_timeout_succeeds_normally() -> Result<()> {
    ...
    let rows = with_timeout(
        ...
        engine
            .connect()
            .await?
            .fetch_all("SELECT pg_sleep(0.1)::text", &[]),
    )
    .await?;
...
async fn a_slow_query_is_cancelled_by_its_timeout() -> Result<()> {
    ...
    let outcome = with_timeout(
        ...
        engine
            .connect()
            .await?
            .fetch_all("SELECT pg_sleep(3)::text", &[]),
    )
    .await;
    assert!(
...
async fn aborting_the_task_running_a_slow_query_also_cancels_it() -> Result<()> {
    ...
    let handle = tokio::spawn(async move {
        engine
            .connect()
            .await?
            .fetch_all("SELECT pg_sleep(5)::text", &[])
            .await
    });
