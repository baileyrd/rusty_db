...
//! Same coverage as `audit_log.rs` (SQLite), against a real Postgres
//! server. Each test uses its own table *and* its own audit table name
//! (`with_audit_log_table`) to avoid colliding with other tests running
//! concurrently against the same shared live server.
...
async fn insert_update_and_delete_are_each_recorded() -> rusty_db::Result<()> {
    ...
    recreate_table(&engine, "audit_log_pg_users").await?;
    engine
        .connect()
        .await?
        .execute("DROP TABLE IF EXISTS audit_log_pg_history", &[])
        .await?;

    let mut session = engine
        .session()
        .with_audit_log_table("audit_log_pg_history");
    ...
    assert_eq!(log.len(), 3);
    assert_eq!(log[0].table, "audit_log_pg_users");
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE audit_log_pg_users", &[])
        .await?;
    engine
        .connect()
        .await?
        .execute("DROP TABLE audit_log_pg_history", &[])
        .await?;
...
async fn audit_entries_share_atomicity_with_the_write_they_record() -> rusty_db::Result<()> {
    ...
    recreate_table(&engine, "audit_log_pg_atomicity_users").await?;
    engine
        .connect()
        .await?
        .execute("DROP TABLE IF EXISTS audit_log_pg_atomicity_history", &[])
        .await?;

    #[derive(Debug, Clone, PartialEq, Mapped)]
    #[table(name = "audit_log_pg_atomicity_users")]
    ...
    let mut session = engine
        .session()
        .with_audit_log_table("audit_log_pg_atomicity_history");
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE audit_log_pg_atomicity_users", &[])
        .await?;
    engine
        .connect()
        .await?
        .execute("DROP TABLE audit_log_pg_atomicity_history", &[])
        .await?;
