...
//! Exercises `Value::Decimal`/`BigDecimal` against a real Postgres server,
//! which has a native `NUMERIC` column type — unlike SQLite/MySQL (see
//! `decimal_value.rs`), a column reflected/decoded here should come back
//! as `Value::Decimal` directly, not `Value::Text`.
...
#[table(name = "decimal_value_pg_invoices")]
struct Invoice {
    #[table(primary_key)]
    id: i64,
    total: BigDecimal,
...
#[tokio::test]
async fn decimal_field_round_trips_through_a_native_numeric_column() -> rusty_db::Result<()> {
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE IF EXISTS decimal_value_pg_invoices", &[])
        .await?;
    engine
        .connect()
        .await?
        .execute(
            "CREATE TABLE decimal_value_pg_invoices (\
                 id BIGINT PRIMARY KEY, total NUMERIC(12,2) NOT NULL, discount NUMERIC(12,2)\
             )",
            &[],
        )
        .await?;
    ...
    let row = engine
        ...
        .await?;
    assert!(
        matches!(row.value(1), Some(Value::Decimal(_))),
        "a native NUMERIC column should decode as Value::Decimal, not Value::Text: {:?}",
        row.value(1)
    );
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE decimal_value_pg_invoices", &[])
        .await?;
    Ok(())
}
...
async fn null_decimal_field_round_trips_as_none() -> rusty_db::Result<()> {
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE IF EXISTS decimal_value_pg_null_invoices", &[])
        .await?;
    engine
        .connect()
        .await?
        .execute(
            "CREATE TABLE decimal_value_pg_null_invoices (\
                 id BIGINT PRIMARY KEY, total NUMERIC(12,2) NOT NULL, discount NUMERIC(12,2)\
             )",
            &[],
        )
    ...
    let table = Table::new("decimal_value_pg_null_invoices");
    engine
        .execute(
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE decimal_value_pg_null_invoices", &[])
        .await?;
    Ok(())
}
