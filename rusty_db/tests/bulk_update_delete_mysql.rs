...
//! MySQL/MariaDB server — just the two round-trip tests, since the
...
#[table(name = "bulk_update_delete_mysql_update_users")]
struct UpdateUser {
    #[table(primary_key)]
    id: i64,
    name: String,
...
async fn bulk_update_changes_every_matching_row_in_one_statement() -> rusty_db::Result<()> {
    ...
    engine
        ...
        .await?
        .execute(
            "DROP TABLE IF EXISTS bulk_update_delete_mysql_update_users",
            &[],
        )
        .await?;
    engine
        ...
        .await?
        .execute(
            "CREATE TABLE bulk_update_delete_mysql_update_users \
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE bulk_update_delete_mysql_update_users", &[])
        .await?;
    Ok(())
}

#[derive(Debug, Clone, PartialEq, Mapped)]
#[table(name = "bulk_update_delete_mysql_delete_users")]
struct DeleteUser {
    #[table(primary_key)]
    id: i64,
    name: String,
...
async fn bulk_delete_removes_every_matching_row_in_one_statement() -> rusty_db::Result<()> {
    ...
    engine
        ...
        .await?
        .execute(
            "DROP TABLE IF EXISTS bulk_update_delete_mysql_delete_users",
            &[],
        )
        .await?;
    engine
        ...
        .await?
        .execute(
            "CREATE TABLE bulk_update_delete_mysql_delete_users \
                 (id BIGINT PRIMARY KEY, name TEXT NOT NULL, active BOOLEAN NOT NULL)",
            &[],
        )
        .await?;
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE bulk_update_delete_mysql_delete_users", &[])
        .await?;
    Ok(())
}
