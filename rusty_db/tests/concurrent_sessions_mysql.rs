...
//! MySQL/MariaDB server. `Session` is intentionally `!Send` (see its
//! identity map), so these run on `tokio::task::LocalSet` + `spawn_local`
//! rather than `tokio::spawn` — the standard way to get genuinely
//! concurrent, interleaved execution of `!Send` futures on one thread.
//!
