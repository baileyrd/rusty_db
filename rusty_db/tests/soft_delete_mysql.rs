...
//! Same coverage as `soft_delete.rs` (SQLite), against a real
//! MySQL/MariaDB server. Each test uses its own table (`#[derive(Mapped)]`'s
...
#[table(name = "soft_delete_mysql_marks_users")]
struct MarksUser {
    #[table(primary_key)]
    id: i64,
    #[table(soft_delete)]
...
async fn deleting_marks_the_row_instead_of_removing_it() -> rusty_db::Result<()> {
    ...
    recreate_table(&engine, "soft_delete_mysql_marks_users").await?;
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE soft_delete_mysql_marks_users", &[])
        .await?;
...
#[derive(Debug, Clone, PartialEq, Mapped)]
#[table(name = "soft_delete_mysql_get_users")]
struct GetUser {
    #[table(primary_key)]
    id: i64,
    #[table(soft_delete)]
...
async fn get_treats_a_soft_deleted_row_as_not_found() -> rusty_db::Result<()> {
    ...
    recreate_table(&engine, "soft_delete_mysql_get_users").await?;
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE soft_delete_mysql_get_users", &[])
        .await?;
