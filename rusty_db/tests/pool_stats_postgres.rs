...
//! A reduced version of `pool_stats.rs` (SQLite) against a real Postgres
//! server — just the two tests that most directly prove `pool_stats()`
//! against a real network-backed pool (checkout/release and blocked-acquire
//! waiters); the pure counting behavior (`total_acquires` accumulating
//! across several checkouts, a fresh pool's zeroed counters) is already
//! covered there and doesn't need re-proving per driver.
