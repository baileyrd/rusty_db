...
//! Same coverage as `backup_restore.rs` (SQLite), against a real
//! MySQL/MariaDB server.
//!
//! Unlike the SQLite version (a fresh, isolated file-backed database per
//! test), this shares one live server with every other MySQL test in the
//! suite. `Engine::backup`/`restore` operating on *every* table would
