...
//! A reduced version of `savepoints.rs` (SQLite) against a real Postgres
//! server — just the two tests that most directly prove `SAVEPOINT`/
//! `ROLLBACK TO SAVEPOINT`/`RELEASE SAVEPOINT` actually work against a
//! real server, since the rest (nested savepoints, discarding
...
#[table(name = "savepoints_pg_rollback_users")]
struct RollbackUser {
    #[table(primary_key)]
    id: i64,
    name: String,
...
async fn rollback_to_savepoint_undoes_a_sub_unit_of_work_without_aborting_the_transaction(
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE IF EXISTS savepoints_pg_rollback_users", &[])
        .await?;
    engine
        .connect()
        .await?
        .execute(
            "CREATE TABLE savepoints_pg_rollback_users (id BIGINT PRIMARY KEY, name TEXT NOT NULL)",
            &[],
        )
        .await?;
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE savepoints_pg_rollback_users", &[])
        .await?;
    Ok(())
}

#[derive(Debug, Clone, PartialEq, Mapped)]
#[table(name = "savepoints_pg_release_users")]
struct ReleaseUser {
    #[table(primary_key)]
    id: i64,
    name: String,
...
async fn release_savepoint_keeps_its_effects_and_the_transaction_continues() -> rusty_db::Result<()>
{
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE IF EXISTS savepoints_pg_release_users", &[])
        .await?;
    engine
        .connect()
        .await?
        .execute(
            "CREATE TABLE savepoints_pg_release_users (id BIGINT PRIMARY KEY, name TEXT NOT NULL)",
            &[],
        )
        .await?;
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE savepoints_pg_release_users", &[])
        .await?;
    Ok(())
}
