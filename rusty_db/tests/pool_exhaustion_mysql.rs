...
//! Same coverage as `pool_exhaustion.rs` (SQLite), against a real
//! MySQL/MariaDB server: `PoolConfig` lets these constrain `max_connections`
//! (and `acquire_timeout`) so exhaustion is triggered deterministically
//! rather than by guessing how much concurrent load a default-sized pool
//! can absorb.
