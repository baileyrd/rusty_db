...
//! Same coverage as `bulk_insert.rs` (SQLite), against a real
...
#[table(name = "bulk_insert_mysql_round_trip")]
...
async fn session_add_all_inserts_every_row_in_one_round_trip() -> rusty_db::Result<()> {
    ...
    recreate_table(&engine, "bulk_insert_mysql_round_trip").await?;
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE bulk_insert_mysql_round_trip", &[])
        .await?;
...
#[derive(Debug, Clone, PartialEq, Mapped)]
#[table(name = "bulk_insert_mysql_conflict")]
struct ConflictUser {
    #[table(primary_key)]
    id: i64,
    name: String,
...
async fn a_failing_bulk_insert_rolls_back_the_whole_batch() -> rusty_db::Result<()> {
    ...
    recreate_table(&engine, "bulk_insert_mysql_conflict").await?;
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE bulk_insert_mysql_conflict", &[])
        .await?;
