...
//! against a real Postgres server, which has native `DATE`/`TIME`/
//! `TIMESTAMP`/`TIMESTAMPTZ` column types — unlike SQLite (see
//! `temporal_value.rs`), a column reflected/decoded here should come back
//! as the matching native variant directly, not `Value::Text`.
...
#[table(name = "temporal_value_pg_events")]
struct Event {
    #[table(primary_key)]
    id: i64,
    day: NaiveDate,
...
async fn temporal_fields_round_trip_through_native_column_types() -> rusty_db::Result<()> {
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE IF EXISTS temporal_value_pg_events", &[])
        .await?;
    engine
        .connect()
        .await?
        .execute(
            "CREATE TABLE temporal_value_pg_events (\
                 id BIGINT PRIMARY KEY, day DATE NOT NULL, time_of_day TIME NOT NULL, \
                 logged_at TIMESTAMP NOT NULL, happened_at TIMESTAMPTZ NOT NULL, \
                 canceled_on DATE\
             )",
            &[],
        )
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE temporal_value_pg_events", &[])
        .await?;
    Ok(())
}
...
async fn null_temporal_field_round_trips_as_none() -> rusty_db::Result<()> {
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE IF EXISTS temporal_value_pg_null_events", &[])
        .await?;
    engine
        .connect()
        .await?
        .execute(
            "CREATE TABLE temporal_value_pg_null_events (\
                 id BIGINT PRIMARY KEY, day DATE NOT NULL, time_of_day TIME NOT NULL, \
                 logged_at TIMESTAMP NOT NULL, happened_at TIMESTAMPTZ NOT NULL, \
                 canceled_on DATE\
             )",
    ...
    let table = Table::new("temporal_value_pg_null_events");
    engine
        .execute(
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE temporal_value_pg_null_events", &[])
        .await?;
    Ok(())
}
