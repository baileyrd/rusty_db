...
#[table(name = "optimistic_locking_mysql_update")]
...
async fn updating_with_a_stale_version_fails_with_conflict() -> rusty_db::Result<()> {
    ...
    recreate_table(&engine, "optimistic_locking_mysql_update").await?;
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE optimistic_locking_mysql_update", &[])
        .await?;
...
#[derive(Debug, Clone, PartialEq, Mapped)]
#[table(name = "optimistic_locking_mysql_delete")]
...
async fn deleting_with_a_stale_version_fails_with_conflict() -> rusty_db::Result<()> {
    ...
    recreate_table(&engine, "optimistic_locking_mysql_delete").await?;
    ...
    engine
        .connect()
        .await?
        .execute("DROP TABLE optimistic_locking_mysql_delete", &[])
        .await?;
